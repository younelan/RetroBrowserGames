class Player {
  constructor(x, y, z, team, color, position, scene) {
    ...
    // Game stats
    this.speed = 18; // Increased from 13 for more dynamic gameplay
  ...
  createModel(x, y, z, scene) {
    ...
    const skinColor = 0xf8d8c0;

    // TORSO (Jersey)
    const bodyGeometry = new THREE.CylinderGeometry(0.8, 0.7, 2.5, 8);
    const bodyMaterial = new THREE.MeshStandardMaterial({ color: primaryColor, roughness: 0.5 });
    ...
    // NECK
    const neckGeo = new THREE.CylinderGeometry(0.2, 0.2, 0.4, 8);
    ...
    // HAIR / HEADBAND
    const hairGeo = new THREE.CylinderGeometry(0.68, 0.68, 0.3, 16);
    ...
    this.leftArm.position.set(-1.0, 3.0, 0);
    this.leftArm.rotation.z = Math.PI / 12;
    this.mesh.add(this.leftArm);

    this.rightArm = new THREE.Mesh(armGeo, armMat);
    this.rightArm.position.set(1.0, 3.0, 0);
    this.rightArm.rotation.z = -Math.PI / 12;
    this.mesh.add(this.rightArm);

    // LEGS with SHORTS
    const legGeo = new THREE.CylinderGeometry(0.3, 0.2, 2.5, 8);
    const shortMat = new THREE.MeshStandardMaterial({ color: secondaryColor });
    const legSkinMat = new THREE.MeshStandardMaterial({ color: skinColor });

    this.leftLeg = new THREE.Group();
    this.leftLeg.position.set(-0.4, 1.25, 0);
    this.mesh.add(this.leftLeg);

    const lShort = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.35, 1.2, 8), shortMat);
    lShort.position.y = 0.6;
    this.leftLeg.add(lShort);

    const lLegPart = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.2, 1.3, 8), legSkinMat);
    lLegPart.position.y = -0.6;
    this.leftLeg.add(lLegPart);

    this.rightLeg = new THREE.Group();
    this.rightLeg.position.set(0.4, 1.25, 0);
    this.mesh.add(this.rightLeg);

    const rShort = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.35, 1.2, 8), shortMat);
    rShort.position.y = 0.6;
    this.rightLeg.add(rShort);

    const rLegPart = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.2, 1.3, 8), legSkinMat);
    rLegPart.position.y = -0.6;
    this.rightLeg.add(rLegPart);

    // SHOES
    const shoeGeo = new THREE.BoxGeometry(0.5, 0.3, 0.8);
    const shoeMat = new THREE.MeshStandardMaterial({ color: 0xffffff });
    const lShoe = new THREE.Mesh(shoeGeo, shoeMat);
    lShoe.position.set(0, -1.2, 0.2);
    this.leftLeg.add(lShoe);
    const rShoe = new THREE.Mesh(shoeGeo, shoeMat);
    rShoe.position.set(0, -1.2, 0.2);
  ...
  handleUserInput(deltaTime, gameState) {
    ...
    // SHOOT: Handle shooting when player has the ball (Space or F)
    const shootPressed = keys[' '] || keys['f'] || keys['F'];
    if (shootPressed && this.hasBall && this.shootCooldown <= 0) {
      this.shootTheBall(gameState);
    }
    // JUMP: Space key for jumping (only if not shooting)
    else if (keys[' '] && !this.isJumping && this.jumpCooldown <= 0 && !this.hasBall) {
      this.jump();
    }
  ...
  shootTheBall(gameState) {
    // Just use the improved shoot method which is already being used by AI
